...
import class_schedule as class_schedule_module
...
# =============================================================
# Tool: class_schedule — 小如的课程表查询
# 数据源：config.yaml 的 class_schedule 段（dashboard 课程表 tab 编辑）
# =============================================================
@mcp.tool()
async def class_schedule(
    ...
    """查询小如的课程表。不传参数返回今天课表和当前节次状态；传 date 查指定日期课表；传 days 查从 date 起连续多天。日期格式 YYYY-MM-DD。"""
    from datetime import date as _date_type
    try:
        if not str(date or "").strip() and days <= 0:
            return class_schedule_module.today_summary()
        ...
        if str(date or "").strip():
            try:
                ...
                return '日期格式没看懂，请用 YYYY-MM-DD，比如 class_schedule(date="2026-09-01")。'

        if days > 0:
            return class_schedule_module.range_summary(target, min(int(days), 90))

        info = class_schedule_module.schedule_for_date(target)
        if info["override"] is not None:
            label = info["override"].get("label") or "特殊安排"
            return f"{info['date']}（{info['weekday']}）{label}，无常规课程"
        if not info["items"]:
            return f"{info['date']}（{info['weekday']}）没有课程安排"
        lines = [f"{info['date']}（{info['weekday']}）小如的课程："]
        lines += [f"  {item['start']}–{item['end']} {item['name']}" for item in info["items"]]
        return "\n".join(lines)
    except Exception as exc:
        logger.error("class_schedule tool error: %s", exc, exc_info=True)
        return f"课程表查询出错了：{exc}"
...
async def api_config_get(request):
    ...
    return JSONResponse({
        ...
        "class_schedule": config.get("class_schedule") if isinstance(config.get("class_schedule"), dict) else {},
        "self_anchor": {
...
async def api_config_update(request):
    ...
    # --- Class schedule config（课程表；gateway 侧通过读 config.yaml 获取，
    #     因此本段必须持久化写入才能让聊天注入生效，dashboard 端强制 persist）---
    if "class_schedule" in body and isinstance(body["class_schedule"], dict):
        cs_payload = body["class_schedule"]
        cleaned_schedule: dict = {}
        if "enabled" in cs_payload:
            cleaned_schedule["enabled"] = bool(cs_payload.get("enabled", True))
        for _cs_key in ("owner", "term_start"):
            if _cs_key in cs_payload:
                cleaned_schedule[_cs_key] = str(cs_payload.get(_cs_key) or "").strip()
        if isinstance(cs_payload.get("weekly"), dict):
            cleaned_weekly: dict = {}
            for _cs_day in ("mon", "tue", "wed", "thu", "fri", "sat", "sun"):
                _cs_items = cs_payload["weekly"].get(_cs_day)
                if isinstance(_cs_items, list):
                    cleaned_day_items = []
                    for _cs_item in _cs_items:
                        if not isinstance(_cs_item, dict):
                            continue
                        _cs_name = str(_cs_item.get("name") or "").strip()[:40]
                        _cs_start = str(_cs_item.get("start") or "").strip()
                        _cs_end = str(_cs_item.get("end") or "").strip()
                        if _cs_name and _cs_start and _cs_end:
                            cleaned_day_items.append({"name": _cs_name, "start": _cs_start, "end": _cs_end})
                    cleaned_weekly[_cs_day] = cleaned_day_items
            cleaned_schedule["weekly"] = cleaned_weekly
        if isinstance(cs_payload.get("overrides"), dict):
            cleaned_overrides: dict = {}
            for _cs_date, _cs_info in cs_payload["overrides"].items():
                _cs_date_str = str(_cs_date or "").strip()
                if not _cs_date_str:
                    continue
                if isinstance(_cs_info, dict):
                    cleaned_overrides[_cs_date_str] = {
                        "label": str(_cs_info.get("label") or "").strip()[:60],
                        "kind": str(_cs_info.get("kind") or "holiday").strip(),
                    }
            cleaned_schedule["overrides"] = cleaned_overrides
        config["class_schedule"] = cleaned_schedule
        updated.append("class_schedule")
        # 课表立即写盘：gateway 进程通过读 config.yaml 获取课表（mtime 缓存自动感知），
        # 这里不依赖 persist 标志，POST 一到就落盘——修复「保存后刷新丢失」
        try:
            _cs_path = os.environ.get(
                "OMBRE_CONFIG_PATH",
                os.path.join(os.path.dirname(os.path.abspath(__file__)), "config.yaml"),
            )
            _cs_existing: dict = {}
            if os.path.exists(_cs_path):
                with open(_cs_path, "r", encoding="utf-8") as _cs_f:
                    _cs_existing = yaml.safe_load(_cs_f) or {}
            _cs_existing["class_schedule"] = cleaned_schedule
            with open(_cs_path, "w", encoding="utf-8") as _cs_f:
                yaml.dump(_cs_existing, _cs_f, default_flow_style=False, allow_unicode=True, sort_keys=False)
            logger.info("Class schedule saved immediately to %s", _cs_path)
        except Exception as _cs_exc:
            logger.warning("Class schedule immediate persist failed: %s", _cs_exc)
    ...
    if body.get("persist", False):
        ...
        def _apply_dashboard_config(save_config: dict) -> dict:
            ...
            if "class_schedule" in body and isinstance(body["class_schedule"], dict):
                save_config["class_schedule"] = body["class_schedule"]
